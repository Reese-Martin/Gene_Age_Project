...
# v2 removes old code, and transitions to violin plots
...
def AgeSortedArray(df, trait):
    ...
    Age_BP[:, 1] = np.asarray([len(df['Assc_BioProcesses'][i]) if df[trait][i] == 1 else 0 for i in df.index])
...
def Allfuncviolins(df):
    Age_BP = np.zeros([len(df), 2])
    Age_BP[:, 0] = np.asarray(list(df['AgeBin']))
    Age_BP[:, 1] = np.asarray([len(df['Assc_BioProcesses'][i]) for i in df.index])
    Age_BP_sorted = Age_BP[np.argsort(Age_BP[:, 0])]
    Age_BP_sorted[np.argwhere(Age_BP_sorted[:, 1] == 0), 1] = np.nan
...
def anova_2w(x):
    ...
    df = pd.DataFrame(columns=['BPs', 'Ages', 'traits'], data=zip(BPs, Ages, traits))

    lm = ols('BPs~Ages+C(traits)', data=df).fit()
    table = sm.stats.anova_lm(lm)
    print(table)
    Age_pval = table['PR(>F)'][['Ages', 'C(traits)']].values
...
# count processes separated by function and calculate SEM
...
cntr = 0
for ax in axes.flat:
    # tmp = [len(i) for i in Data[cntr][0]]
    # old_widths = [i/max(tmp) for i in tmp]
    # old_widths = [i+.4 if i < 1 else i for i in old_widths]
    vp = ax.violinplot(Data[cntr][0], pos[cntr], showmeans=False, showmedians=False, showextrema=False)
    # tmp = [len(i) for i in Data[cntr][-1]]
    # young_widths = [i/max(tmp) for i in tmp]
    # young_widths = [i+.4 if i < 1 else i for i in young_widths]
    ...
    ax.set_ylim([0, 5.5])
    ax.plot([lineXloc[cntr], lineXloc[cntr]], [0, 5.5], color='black')
    cntr2 = 0
    for pc in vp['bodies']:
...
fig.show()
fig.savefig(image_folder+"F2_Age_vs_BP_funcs_boxplot.tiff", dpi=300)
...
fig.legend(handles=[all, met, cell, Dev, Imm], loc='right')
fig.show()
fig.savefig(image_folder+"S4_Age_vs_BP_bootstrap_means.tiff")
