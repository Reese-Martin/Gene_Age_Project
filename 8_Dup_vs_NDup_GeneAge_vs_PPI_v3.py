...
def anova_2w(x, x2):
    ...
    df = pd.DataFrame(columns=['Ages', 'PPIs', 'status'], data=data)

    lm = ols('PPIs~Ages+C(status)', data=df).fit()
...
def D_AgeSortedArray(df):
    Age_BP = np.zeros([len(df[df['Paralogs'] > 0]), 2])
    Age_BP[:, 0] = np.asarray([df.loc[i, 'AgeBin'] for i in df.index if df.loc[i, 'Paralogs'] > 0])
    # Age_BP[:, 1] = np.asarray([np.asarray(df.loc[i, 'MedConf_PPI']) + np.asarray(df.loc[i, 'HighConf_PPI']) for i in df.index if df.loc[i, 'Paralogs'] > 0])
    Age_BP[:, 1] = np.asarray([df.loc[i, 'HighConf_PPI'] for i in df.index if df.loc[i, 'Paralogs'] > 0])
...
D_HAge_Sorted = D_AgeSortedArray(Hdf)
...
ND_HAge_Sorted = ND_AgeSortedArray(Hdf)
...
D_Mage_Sorted = D_AgeSortedArray(Mdf)
...
ND_MAge_Sorted = ND_AgeSortedArray(Mdf)
...
D_Dage_Sorted = D_AgeSortedArray(Ddf)
...
ND_DAge_Sorted = ND_AgeSortedArray(Ddf)
...
D_Drage_Sorted = D_AgeSortedArray(Drdf)
...
ND_DRAge_Sorted = ND_AgeSortedArray(Drdf)
...
for ax in axes.flat:
    if cntr < 5:
        ...
        ax.plot(D_Meds[cntr][:, 0]+1.5, D_Meds[cntr][:, 1], c='orange', linewidth=1,alpha=1)
        ax.set_ylim([0, 8.5])
...
fig.show()
fig.savefig(image_folder+"S3_Age_vs_PPI_paralogs.tiff")
# fig.savefig(image_folder+"S3_Age_vs_PPI_paralogs_HConf.tiff")
